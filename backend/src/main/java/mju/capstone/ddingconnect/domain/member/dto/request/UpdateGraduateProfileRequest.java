...
/**
 * [졸업생 마이페이지 프로필 수정 요청 DTO]
 * GRADUATE 역할 전용. 공통 9필드 + GRADUATE 전용 4필드(명함이미지·직무·회사명·경력)만 노출한다.
 * STUDENT 전용 필드(grade)는 정의 자체가 없다.
 *
 * 부분 수정 규약 — 필드가 null 이면 해당 항목은 변경하지 않는다.
 * 형식 검증: 이메일은 @mju.ac.kr 도메인, githubLink/linkedinLink 는 각 도메인 URL.
 */
public record UpdateGraduateProfileRequest(
     ...
     * STUDENT 전용 필드(grade)는 null 로 채워, validateRoleFields 가 GRADUATE 진입 시 통과하도록 한다.
     */
