...
- **매칭은 무상태 pass-through**: 매칭 입력값·결과는 저장하지 않으며 매칭 이력 엔티티가 없다. 알고리즘 응답(점수)도 저장하지 않으며 백엔드 메모리에서 정렬·top N 후 ID 리스트로만 변환해 반환 — `CoffeeChat.jobScore/ability/goal` 은 계속 미사용 컬럼.
- **매칭 알고리즘 연동 (`MatchingAlgorithmClient` / `MatchingAlgorithmClientImpl`)**: 데이터 파트의 1대1 페어 매칭 알고리즘과 정합한다 — 백엔드가 **양방향 페어** `{requester: UserInfo, receiver: UserInfo}` 를 후보 N명만큼 N회 호출하고, 응답 점수(`totalMatchRate`)를 받아 **백엔드가 직접 정렬·top N 선택**한다. base URL 은 설정값 `matching.algorithm.base-url`(env `MATCHING_ALGORITHM_BASE_URL` 오버라이드, 기본 `http://localhost:8000`), 상위 N 은 `matching.algorithm.top-n`(env `MATCHING_ALGORITHM_TOP_N`, 기본 `3`). 엔드포인트 경로는 `POST /api/data/coffeechat/match` (`MatchingAlgorithmClientImpl.MATCH_ENDPOINT_PATH` 상수). 호출 실패·타임아웃·빈 응답은 모두 502 (`MATCHING_ALGORITHM_FAILED`).
- **매칭 후보 풀 구성 (학생 → 졸업생 한정)**: `MemberRepository.findAllByRole(GRADUATE)` 로 졸업생 전체를 가져와 (1) 신청자 본인 제외, (2) 매칭용 데이터가 비어 있는 후보(`Graduate.jobType` null, `Graduate.company` blank, 본인 `TechStack` 0건) 제외. 후보 0명이면 알고리즘 호출 없이 빈 리스트 반환.
- **`UserInfo` 매핑 규약**: 데이터 파트 스키마는 `{user_id, job, tech_stacks, goal}` (snake_case, 대문자 enum 가정). 요청자(학생, 폼) 측 — `user_id = requesterId`, `job = form.interestedJob`(원본 pass-through, 프론트가 enum 명 송신 가정), `tech_stacks = form.capability` 콤마 split → trim → 대문자 → `TechStackName` 화이트리스트 필터(미지원 토큰 버림) → 중복 제거, `goal = form.targetCompany`. 후보(졸업생, DB) 측 — `user_id = member.id`, `job = graduate.jobType.name()`, `tech_stacks = TechStackRepository.findByMemberId(...).stream().map(TechStackName::name)`, `goal = graduate.company`.
- **응답 스키마 정규화**: 데이터 파트 응답 `{status, match_results: {jobScore, ability, goal, totalMatchRate}}` 은 내부 record `AlgorithmMatchResponse`/`MatchScores` 로 받는다. 응답 키 네이밍 혼용(`jobScore`·`totalMatchRate` camelCase / `ability`·`goal` 소문자 / envelope `match_results` snake_case) 은 record 필드명·`@JsonProperty` 로 명시 매핑. 점수는 응답 DTO 에 노출하지 않고 정렬 키로만 사용 — `topMatches` 는 계속 `List<Long>` 반환(`CoffeeChatMatchingServiceImpl` 가 `memberIds` 만 사용하는 흐름 유지).
