...
- **매칭 알고리즘 연동 (`MatchingAlgorithmClient` / `MatchingAlgorithmClientImpl`)**: 백엔드가 폼 6필드 + 신청자 ID 를 외부 유사도 알고리즘(타 파트)에 `RestClient` 로 전달하고 후보 회원 ID 리스트를 받는다. base URL 은 설정값 `matching.algorithm.base-url`(env `MATCHING_ALGORITHM_BASE_URL` 오버라이드, 기본 `http://localhost:8000`), 엔드포인트 경로는 `POST /api/data/coffeechat/match` (`MatchingAlgorithmClientImpl.MATCH_ENDPOINT_PATH` 상수). 호출 실패·타임아웃·빈 응답은 모두 502 (`MATCHING_ALGORITHM_FAILED`). **타 파트 협의 미확정**: 요청/응답 JSON 스키마 — `MatchingAlgorithmClientImpl` 내부 `record`(`AlgorithmMatchRequest`/`AlgorithmMatchResponse`) 매핑만 확정 후 교체하면 된다.
