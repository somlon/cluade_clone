...
- **커피챗 매칭 플로우 (`CoffeeChatMatchingController` / `CoffeeChatMatchingService(Impl)`)**: Figma 매칭 화면(정보 입력 → 결과 리스트 → 상대 상세)의 백엔드. 신규 컨트롤러는 base `/api/v1/coffeechat` 로 기존 `CoffeeChatController` 와 메서드 경로가 달라 충돌 없음. 엔드포인트 3개:
  ...
  - `GET /api/v1/coffeechat/my-activity` — 로그인 회원이 신청자이고 `status=ACCEPTED` 인 커피챗 상대 목록을 카드 UI 전용 경량 DTO(`MyActivityCoffeeChatResponse`, 8필드)로 반환. 없으면 빈 리스트.
...
- **후보 프로필 조립 (`CandidateProfileAssembler`)**: `memberId` → 카드/상세/나의활동 DTO 조립(매칭 결과·상대 상세·나의 활동 공통 로직). 회원 공통 정보 + 역할별 부가 정보(GRADUATE = 입학연도·회사·`jobType`·경력, 상세는 명함·공고 추가 / STUDENT = 학년)를 모은다. 입학연도는 `Member.studentNumber` 의 3·4번째 자리(0-index 2~3) 파생(별도 컬럼 없음, 4자리 미만이면 null). 직군은 `TargetJobRepository.findByMemberId`, 기술스택은 `TechStackRepository.findByMemberId` 로 전체 리스트 조회(0개면 빈 리스트). 타 도메인 레포 직접 주입은 조회 전용 조립 목적이며 QnA 선례상 허용. 메서드 3개 — `assembleCard`(매칭 카드) / `assembleDetail`(상대 상세) / `assembleMyActivityCard`(나의 활동 경량 8필드, GRADUATE 가드 후 `Graduate.jobType` 포함). 응답 DTO `MatchedCandidateResponse`(카드)/`MatchedCandidateDetailResponse`(상세)는 한 record 에 역할별 nullable 필드를 두는 `MemberResponse` 패턴 — 상세에 "나와의 커피챗 상태"는 넣지 않는다(수락/거절은 기존 커피챗 알람으로 전달).
- **`region` 필드 전면 제거**: `MatchedCandidateResponse`·`MatchedCandidateDetailResponse` 의 `region` 은 어셈블러가 항상 null 로 채우던 "소스 미확정" 더미였고 UI 가 지역을 표시하지 않기로 확정돼 **키 자체를 삭제**했다(매칭 카드/상세 모두 미노출 — 항상 null 이라 화면 영향 없음).
- **나의 활동 커피챗 경량 DTO (`MyActivityCoffeeChatResponse`)**: `getMyActivity` 는 매칭 상세 DTO 대신 카드 UI 가 실제 쓰는 8필드(`memberId · nickname · department · enrollmentYear · company · jobType · careerYear · techStacks`)만 담은 전용 경량 DTO 를 반환한다 — 화면이 안 쓰는 상세 필드(`portfolio`/`githubLink`/`linkedinLink`/`businessCardImage`/`jobPosts`/`jobCategories`/`grade`)를 내려보내지 않아 페이로드를 줄이고 책임을 분리한다. 화면 필수인 **`jobType`(`Graduate.jobType`, `JobType` enum)** 은 매칭 카드/상세에는 없고 이 경량 카드에서만 노출된다.
